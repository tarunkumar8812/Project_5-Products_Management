...
async function createCart(req, res) {
  try {
    ...
    if (Object.keys(rest).length > 0) return res.status(400).send({ status: false, message: `You can not fill these:- ( ${Object.keys(rest)} )field`, });
    ...
    if (!ObjectId.isValid(userId)) return res.status(400).send({ status: false, message: "required valid userId" });


    // --------------------validation of CartId--------------------

    if (cartId) {
      if (!isValidString(cartId)) return res.status(400).send({ status: false, message: "the value of cartId must be string", });

      if (!ObjectId.isValid(cartId.trim())) return res.status(400).send({ status: false, message: "required valid cartId" });
    }


    // --------------------validation of productId--------------------
    if (!isValidString(productId)) return res.status(400).send({ status: false, message: "the value of productId must be string", });

    if (!ObjectId.isValid(productId.trim())) return res.status(400).send({ status: false, message: "required valid productId" });


    // ------------ checking user in DB -------------
    let user_in_DB = await userModel.findById(userId);

    if (!user_in_DB) return res.status(404).send({ status: false, message: "user not found" });
    ...
    // // ----------------- authorization -----------------
    // if (userId !== req.decoded.userId) return res.status(401).send({ status: false, message: "not auth" });
    ...
    if (cartOfUser) {
      ...
      if (cartId) {
        let cart = await cartModel.findOne({ _id: cartId.trim(), userId: userId, });

        if (!cart) return res.status(404).send({ status: false, message: `cart not found with given userID and cartId combination, In place of cartId value use these cartId:-${cartOfUser._id}`, });
        ...
        let upCart = await cartModel.findOneAndUpdate({ _id: cartId.trim(), userId: userId }, updateData, { new: true });
      ...
      return res.status(400).send({ status: false, message: `there is already a cart is there for the give user, so required cartId(${cartOfUser._id}) in request body`, });
    }
...
async function updateCart(req, res) {
  try {
    ...
    if (typeof removeProduct !== "number" || !/^[0-1]$/.test(removeProduct)) return res.status(400).send({ status: false, message: "removeProduct must be number(0/1)" });
    ...
    // if (userId !== req.decoded.userId) return res.status(403).send({ status: false, message: "not auth" });
    ...
    let cartProducts = await productModel.find({ _id: { $in: carProductIds }, });
    ...
    if (data.removeProduct === 1) {
      for (let i = 0; i < cart.items.length; i++) {
        ...
        if (cart.items[i].quantity === 0) {
          let items = cart.items.filter((x) => x.productId.toString() !== data.productId.trim());

          let updatedCart = await cartModel.findOneAndUpdate({ _id: data.cartId.trim(), userId: userId }, { items: items, totalPrice: cart.totalPrice, totalItems: items.length, }, { new: true });
    ...
    let proIds = carProductIds.filter((x) => x.toString() !== data.productId.trim());

    //Making a DB call to get all the products in a variable as a array of objects
    let Products = await productModel.find({ _id: { $in: proIds }, });
    ...
    //updating the cart
    let updatedCart = await cartModel.findOneAndUpdate({ _id: data.cartId.trim(), userId: userId }, { items: items, totalPrice: totalPrice, totalItems: totalItems }, { new: true });
...
async function deleteCart(req, res) {
  try {
    ...
    let cart_in_DB = await cartModel.findOneAndUpdate({ userId: userId }, { items: [], totalPrice: 0, totalItems: 0, }, { new: true });
